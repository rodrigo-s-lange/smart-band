...
def test_reset_is_deterministic_and_shared(tmp_path) -> None:
    ...
    assert len(first.snapshot()["bands"]) == 32
    assert len(second.snapshot()["gateways"]) == 8
    ...
    assert second.snapshot()["active_band"]["participant_name"] == "João Demo"
    assert third.snapshot()["active_band"]["participant_name"] == "João Demo"
    second.reset(NOW)
    assert first.snapshot()["bands"][0]["status"] == "available"
    assert third.snapshot()["interactions"] == []
...
def test_radio_fallback_preserves_interaction_and_tamper_is_audited(tmp_path) -> None:
    ...
    before = store.snapshot()["active_interaction"]
    assert store.simulate_radio_fallback(interaction_id, NOW)
    after = store.snapshot()["active_interaction"]
